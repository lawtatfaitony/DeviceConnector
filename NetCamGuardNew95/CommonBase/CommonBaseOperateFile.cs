...
namespace Common
{
    public partial class CommonBase
    {
        ...
        {
            string loggerline = string.Format("[{0:yyyy-MM-dd HH:mm:ss fff}] [{1}]",DateTime.Now, LineContent);
            bool result = WriteLogNtimes(loggerline); 
            return result;
        }
        public static bool OperateDateLoger(string LineContent,LoggerMode loggerMode)
        {
            string loggerline = string.Format("[{0:yyyy-MM-dd HH:mm:ss fff}] [{1}]", DateTime.Now, LineContent);
            bool result = WriteLogNtimes(loggerline, loggerMode); 
            return result;
        }
